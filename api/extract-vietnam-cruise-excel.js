...
const N8N_WEBHOOK_URL = 'https://aahaas-ai.app.n8n.cloud/webhook/hotel-rate-extract-vn-cruise'
...
const isAllowedVietnamCruiseExcelFile = (filename = '') =>
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!isAllowedVietnamCruiseExcelFile(file.filename)) {
      return res.status(400).json({ error: 'Only Excel files (.xlsx or .xls) are allowed for Vietnam cruise uploads' })
    }

    console.log('Vietnam cruise file received:', file.filename, file.data.length, 'bytes')
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="vietnam_cruise_rates.xlsx"')

    res.send(Buffer.from(response.data))
  } catch (error) {
    console.error('Error calling Vietnam cruise n8n webhook:', error.message)

    if (error.response) {
      console.error('Vietnam cruise n8n error response:', error.response.status)
